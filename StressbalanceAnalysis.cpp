...
ElementMatrix* StressbalanceAnalysis::CreateKMatrixSSAFriction(Element* element){/*{{{*/
   ...
   IssmDouble* alpha2list = xNew<IssmDouble>(NUMVERTICES);
   gauss=element->NewGauss();
   for(int iv=0;iv<NUMVERTICES;iv++){
      gauss->GaussVertex(iv);
      friction->GetAlpha2(&alpha2list[iv],gauss);
   }
   delete gauss;
	...
	for(int ig=gauss->begin();ig<gauss->end();ig++){
      ...
      alpha2 = 0.;
      for(int i=0;i<NUMVERTICES;i++) alpha2+=alpha2list[i]*basis[i];
